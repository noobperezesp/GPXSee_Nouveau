#ifndef MAPSFORGE_DATOMAPA_H
#define MAPSFORGE_DATOMAPA_H
...
private:
	...
	struct PathCTX {
		PathCTX(DatoMapa *data, const RectC &rect, int zoom, QList<Path> *list)
	...
	struct PointCTX {
		PointCTX(DatoMapa *data, const RectC &rect, int zoom, QList<Point> *list)
		  : data(data), rect(rect), zoom(zoom), list(list) {}
	...
	friend HASH_T qHash(const DatoMapa::Key &key);
...
inline HASH_T qHash(const DatoMapa::Key &key)
{
	return ::qHash(key.tile) ^ ::qHash(key.zoom);
}

inline HASH_T qHash(const DatoMapa::Tag &tag)
{
